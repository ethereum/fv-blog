...
Smart contract development is difficult. The code we write is deployed in an extremely adverserial
environment, and must simultaneously secure potentially billions of dollars of other peoples money.
In this context being able to define and prove the correctness of key security properties is
enourmously valuable. Act grew out of the verification work carried out for multi colateral dai and
uniswap v2, and is a ground up rewrite in haskell of the prototype specification language developed
as a part of this process, incorporating many of the lessons learned along the way.
...
equivalence. In the [examples repo](https://github.com/d-xo/act-examples) you can run `dapp build`
and then `act hevm --spec src/simple.act --soljson out/dapp.sol.json` to attempt to prove
equivalence between the spec and the runtime bytecode:
...
Invariants:
  ...
  (((1000 * 1000) <= (pre(Amm.reserve0) * pre(Amm.reserve1))) and ((1000 * 1000) <= (post(Amm.reserve0) * post(Amm.reserve1)))) of Amm violated:
...
If the `Amm` starts with `reserve0 == 2` and `reserve1 == 5000,000`, and `swap0` is called with `amt
...
Invariants:
  ...
  (((1000 * 1000) <= (pre(Amm.reserve0) * pre(Amm.reserve1))) and ((1000 * 1000) <= (post(Amm.reserve0) * post(Amm.reserve1)))) of Amm holds ∎
```
...
Invariants:
  ...
  ((pre(C.x) < 9) and (post(C.x) < 9)) of C violated:
...
The verification is performed in a minimal and well audited kernel. Although occaisionally bugs have
been found in Coq's and other system's kernels, proof in these systems is generally quite strong
evidence.
...
vernacular command, even if they are not in fact recursive. Coq's `Inductive` is analagous to
...
`n` produces the representation of the number `n + 1` (the Successor). To give a concrete example
...
might think of this definition as "unwrapping" the applications of `S`.
...
The best way to understand the system is to run the software your self, and play around with the
