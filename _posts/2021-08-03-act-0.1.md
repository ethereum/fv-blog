...
extremely adverserial environment, and even a single oversight can easily result the loss of
millions of dollars. In this context being able to define and prove the correctness of key security
properties is enourmously valuable. Act grew out of the verification work carried out for multi
colateral dai and uniswap v2, and is a ground up rewrite in haskell of the prototype specification
language developed as a part of this process, incorporating many of the lessons learned along the
way.
...
Two claims are generated for each behaviour, `Pass` and a `Fail`. The `Pass` claim states
...
poststate, if none can be found we have a proof of the base case. For the inductive step we
...
We must first define a helper fact `pow_pred` that states simply that given two integers `a`
