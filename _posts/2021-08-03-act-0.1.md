...
In the case of a `Fail` claim, we can then check that all leaves represent states in which execution
has reverted. Inversely, for a `Pass` claim we can check that in all leaves, storage has been updated as expected and the content of
