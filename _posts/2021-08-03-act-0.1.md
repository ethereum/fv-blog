...
- [Theorem Prover Export](#theorem-prover-export)
...
equivalence between a contracct specification and an implementation of that specification in EVM.
...
all storage updates have occured as defined in the `storage` block, and then ask it to find
...
Similary to dynamic and compound types, while syntax does exist that allows for specification of
...
development.  As we saw in the `AMM` exmaple above, numeric error introduced by precision loss can
